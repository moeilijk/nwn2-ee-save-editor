...
import { createEnvTexture, createSkyBackground } from './sky';
...
export function useThreeScene(onAnimate?: (scene: THREE.Scene) => void): SceneRefs {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(0xc2d9e8, 1500, 5000);
    ...
    const shadowGeom = new THREE.CircleGeometry(1, 64);
    ...
    groundShadow.name = '__ground_shadow';
...
export function clearSceneModels(scene: THREE.Scene) {
  const toRemove = scene.children.filter(
    (c) => !(c instanceof THREE.Light) && c.name !== '__ground_shadow',
  );
  toRemove.forEach((c) => scene.remove(c));

  const shadow = scene.getObjectByName('__ground_shadow');
  if (shadow) {
    shadow.scale.setScalar(0);
...
export function frameBounds(
  ...
  const shadow = scene.getObjectByName('__ground_shadow');
  if (shadow) {
    shadow.position.set(center.x, center.y - size.y * 0.5 - 0.5, center.z);
