...
async fn test_feat_table_loaded() {
    ...
    let table = ctx.loader.get_table("feat")
...
async fn test_feat_columns() {
    ...
    let required_cols = ["LABEL", "FEAT", "PREREQFEAT1", "PREREQFEAT2", "ALLCLASSESCANUSE"];
...
async fn test_feat_prerequisite_chain() {
    ...
    if let Some(pa_row) = power_attack_row {
        ...
        println!("  Prereq1: {:?}, Prereq2: {:?}, MinStr: {:?}", 
    ...
    if let Some(c_row) = cleave_row {
        ...
        if let Some(prereq) = prereq_id {
            ...
            if let Some(pa_row) = power_attack_row {
                assert_eq!(prereq as usize, pa_row, "Cleave should require Power Attack");
            }
        }
    }
}
    ...
    for row_idx in [1, 2, 3, 4, 5] {
        if row_idx < table.row_count() {
            ...
            if let Some(sr) = strref {
                ...
                println!("Feat {}: {} -> {:?}", row_idx, label.as_deref().unwrap_or("?"), name);
            }
        }
    }
}
...
async fn test_epic_feats() {
    ...
    println!("\n=== Epic Feats (first 30) ===");
    println!("{:<5} {:<45} {:>8} {:>10}", "ID", "Label", "MinLevel", "EpicFlag");
    println!("{}", "-".repeat(75));
...
async fn test_epic_feat_categories() {
    ...
    for row_idx in 0..table.row_count() {
        ...
        epic_by_category.entry(category.to_string())
            .or_default()
            .push(label);
    }
...
async fn test_epic_spell_feats() {
    ...
    for (id, label, prereq, min_lvl) in &epic_spell_feats {
        println!("  {} (ID {}): prereq={}, minlvl={}", label, id, prereq, min_lvl);
    }
}
