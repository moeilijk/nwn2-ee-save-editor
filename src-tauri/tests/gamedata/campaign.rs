...
fn load_campaign_globals(campaign: &str) -> Option<String> {
    let path = fixtures_path().join("saves").join(campaign).join("globals.xml");
    if path.exists() {
...
fn test_motb_campaign_globals() {
    ...
    let parser = RustXmlParser::from_string(&content)
        .expect("Failed to parse MOTB globals.xml");
    ...
    for (group_name, group) in overview.quest_groups.iter().take(5) {
        println!("  {}: {} completed, {} active", 
            group_name, group.completed.len(), group.active.len());
    }
}
...
fn test_classic_campaign_globals() {
    ...
    let parser = RustXmlParser::from_string(&content)
        .expect("Failed to parse Classic Campaign globals.xml");
...
fn test_soz_campaign_globals() {
    ...
    let parser = RustXmlParser::from_string(&content)
        .expect("Failed to parse SoZ globals.xml");
...
fn test_westgate_campaign_globals() {
    ...
    let parser = RustXmlParser::from_string(&content)
        .expect("Failed to parse Westgate globals.xml");
...
fn test_compare_campaign_quest_counts() {
    ...
    let campaigns = ["MOTB", "Classic_Campaign", "STORM_Campaign", "Westgate_Campaign"];
    ...
    for campaign in campaigns {
        if let Some(content) = load_campaign_globals(campaign) {
            if let Ok(parser) = RustXmlParser::from_string(&content) {
                ...
                println!("{}: {} vars, {} groups, {} companions", 
                    campaign, 
...
fn test_campaign_companion_influence() {
    ...
    let parser = RustXmlParser::from_string(&content)
        .expect("Failed to parse globals.xml");
    ...
    for (name, status) in sorted {
        println!("  {} ({}) - Influence: {:?}", name, status.recruitment, status.influence);
    }
}
...
fn test_save_directory_structure() {
    ...
    for entry in std::fs::read_dir(&saves_path).unwrap() {
        ...
        if path.is_dir() {
            ...
            println!("  {} - globals:{} playerinfo:{} resgff:{}", 
                campaign_name,
