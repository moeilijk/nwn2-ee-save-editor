...
async fn test_baseitems_table_loaded() {
    ...
    let table = ctx.loader.get_table("baseitems")
...
async fn test_baseitems_columns() {
    ...
    let table = ctx.loader.get_table("baseitems").expect("baseitems.2da missing");
...
async fn test_dump_weapon_types() {
    ...
    println!("\n=== Weapon Types ===");
    println!("{:<4} {:<25} {:<10} {:<10}", "ID", "Label", "WeapType", "NumDice");
    println!("{}", "-".repeat(55));
    ...
    for row_idx in 0..table.row_count() {
        ...
        if weapon_type > 0 {
            ...
            if weapon_count < 30 {
                println!("{:<4} {:<25} {:<10} {:<10}", row_idx, label, weapon_type, num_dice);
            }
            weapon_count += 1;
        }
    }
...
async fn test_equipment_slots() {
    ...
    for (slot, items) in sorted.iter().take(10) {
        println!("  Slot {}: {} items (e.g. {})", 
            slot, items.len(), items.first().unwrap_or(&String::new()));
    }
}
