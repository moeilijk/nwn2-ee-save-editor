...
fn load_storm_player() -> Character {
    ...
    let handler = SaveGameHandler::new(&save_path, false, false)
        .expect("Failed to open STORM_Campaign save");
...
async fn extract_storm_campaign_character() {
    ...
    println!("Alignment:   {} (LC={}, GE={})", overview.alignment_string, overview.alignment.law_chaos, overview.alignment.good_evil);
    println!("Experience:  {} / {}", overview.experience, overview.xp_progress.xp_for_next_level);
    println!("Total Level: {}", overview.total_level);
    println!("HP:          {} / {}", overview.hit_points.current, overview.hit_points.max);
    println!("AC:          {}", overview.armor_class);
    println!("BAB:         {}", overview.base_attack_bonus);
    println!("Saves:       Fort {} / Ref {} / Will {}", overview.saving_throws.fortitude, overview.saving_throws.reflex, overview.saving_throws.will);
    ...
    println!("  Equipment bonuses: STR {:+} DEX {:+} CON {:+} INT {:+} WIS {:+} CHA {:+}",
        abilities.equipment_modifiers.str_mod, abilities.equipment_modifiers.dex_mod,
        abilities.equipment_modifiers.con_mod, abilities.equipment_modifiers.int_mod,
        abilities.equipment_modifiers.wis_mod, abilities.equipment_modifiers.cha_mod);
    ...
    println!("  AC Breakdown:  base={} armor={} shield={} dex={} natural={} dodge={} deflection={} size={} misc={}",
    ...
    println!("  Melee Attack:  {} (BAB {} + ability {} + size {} + equip {} + misc {})",
    ...
    println!("  Ranged Attack: {} (BAB {} + ability {} + size {} + equip {} + misc {})",
    ...
    println!("  Initiative:    {} (dex {} + feat {} + misc {})",
        combat.initiative.total, combat.initiative.dex, combat.initiative.feat, combat.initiative.misc);
    println!("  Movement:      base={} current={} armor_penalty={}",
        combat.movement.base, combat.movement.current, combat.movement.armor_penalty);
    for dr in &combat.damage_reductions {
        println!("  DR:            {}/{}  ({})", dr.amount, dr.bypass, dr.source);
    }
    ...
    for (name, s) in [("Fort", &saves.fortitude), ("Ref ", &saves.reflex), ("Will", &saves.will)] {
        println!(
    ...
    println!("  STR: +{}, DEX: +{}, CON: +{}, INT: +{}, WIS: +{}, CHA: +{}",
        eq_bonuses.str_bonus, eq_bonuses.dex_bonus, eq_bonuses.con_bonus,
        eq_bonuses.int_bonus, eq_bonuses.wis_bonus, eq_bonuses.cha_bonus);
    println!("  Fort: +{}, Ref: +{}, Will: +{}",
        eq_bonuses.fortitude_bonus, eq_bonuses.reflex_bonus, eq_bonuses.will_bonus);
    println!("  AC armor: +{}, shield: +{}, natural: +{}, dodge: +{}, deflection: +{}, generic: +{}",
        eq_bonuses.ac_armor_bonus, eq_bonuses.ac_shield_bonus, eq_bonuses.ac_natural_bonus,
        eq_bonuses.ac_dodge_bonus, eq_bonuses.ac_deflection_bonus, eq_bonuses.ac_bonus);
    ...
    if let Some(equip_list) = character.get_list_owned("Equip_ItemList") {
        for item_struct in &equip_list {
            ...
            println!("  struct_id={:20} tag={:40} base={}", struct_id, tag, base_item);
        }
    }
    ...
    for item in &eq_summary.equipped {
        println!("  slot={:15} name={:30} base_item={}", item.slot, item.name, item.base_item);
    }
