...
fn test_scan_nwn2_data_directory() {
    ...
    if let Some(data_dir) = paths.data() {
        if data_dir.exists() {
            ...
            assert!(stats.contains_key("last_dir_index_time_ms"), "Should track timing");
        } else {
...
fn test_stats_reset() {
    ...
    if let Some(data_dir) = paths.data() {
        if data_dir.exists() {
            ...
            assert!(!walker.get_stats().is_empty(), "Should have stats after scan");
