...
async fn test_cache_builder_with_real_table_structure() {
    ...
    if let Some(table) = ctx.loader.get_table("classes") {
        ...
        println!("Cache built successfully with {} rows from classes.2da", table.row_count());
    }
}
...
async fn test_cache_key_generation_with_real_install() {
    ...
    if let Some(game_folder) = paths.game_folder() {
        mod_state.insert("install_dir".to_string(), 
...
async fn test_cache_manager_with_built_cache() {
    ...
    if let Some(classes_table) = ctx.loader.get_table("classes") {
        ...
        builder.build_cache(tables_data, "test_cache_key".to_string()).unwrap();
...
fn test_cache_key_validation() {
    ...
    builder.build_cache(tables_data, "original_cache_key".to_string()).unwrap();
    
    let mut manager = CacheManager::new(cache_path).unwrap();
    
    assert!(manager.validate_cache_key("original_cache_key".to_string()).unwrap(),
        "Should validate correct key");
    assert!(!manager.validate_cache_key("wrong_key".to_string()).unwrap(),
...
fn test_cache_invalidation() {
    ...
    assert_eq!(stats.get("loaded_sections").and_then(|v| v.as_u64()), Some(0));
...
async fn test_cache_stats_with_real_data() {
    ...
    builder.build_cache(tables_data, "multi_table_key".to_string()).unwrap();
...
fn test_multiple_cache_sections() {
    ...
    builder.build_cache(tables_data, "sections_key".to_string()).unwrap();
    ...
    let override_data = manager.get_table_data("override_table".to_string()).unwrap();
