...
fn test_discover_nwn2_paths() {
    ...
    assert!(!discovery.nwn2_paths.is_empty(), 
...
fn test_timing_breakdown() {
    ...
    assert!(!discovery.timing_breakdown.is_empty(), 
        "Should have timing breakdown");
    
    println!("=== Timing Breakdown ===");
    for timing in &discovery.timing_breakdown {
        println!("  {}: {}ms ({} paths checked, {} found)",
            timing.operation,
            timing.duration_ms,
            timing.paths_checked,
            timing.paths_found
        );
    }
}
...
fn test_steam_path_classification() {
    ...
    for path in &result.nwn2_paths {
        ...
        if is_steam {
            assert!(in_steam_paths, 
                "Steam path {} should be in steam_paths", path);
        }
    }
}
...
fn test_gog_path_classification() {
    ...
    for path in &result.nwn2_paths {
        ...
        if is_gog {
            assert!(in_gog_paths, 
                "GOG path {} should be in gog_paths", path);
        }
    }
}
...
fn test_profile_path_discovery_benchmark() {
    ...
    println!("  Mean: {:.4}s", profile.get("mean_seconds").unwrap_or(&0.0));
    ...
    assert!(mean < 5.0, "Path discovery should complete in under 5 seconds");
}
...
fn test_discovered_path_is_valid_installation() {
    ...
    for path_str in &result.nwn2_paths {
        ...
        let has_exe = path.join("nwn2main.exe").exists() 
            || path.join("nwn2.exe").exists();
        ...
        println!("Validated: {} (data={}, tlk={}, exe={}, enhanced={})",
...
fn test_no_duplicate_paths() {
    ...
    for path in &result.nwn2_paths {
        ...
        assert!(seen.insert(canonical_str.clone()), 
...
fn test_discovered_paths_match_nwn2_paths() {
    ...
    if let Some(game_folder) = paths.game_folder() {
        ...
        assert!(found, 
            "NWN2Paths game folder should be in discovered paths");
    }
}
...
fn test_is_enhanced_edition() {
    ...
    assert_eq!(is_enhanced, has_enhanced_dir, 
...
fn test_is_steam_installation() {
    ...
    if let Some(game_folder) = paths.game_folder() {
        let path_contains_steam = game_folder.to_string_lossy().to_lowercase().contains("steam");
