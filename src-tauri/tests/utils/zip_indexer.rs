...
fn test_index_nwn2_base_zip() {
    ...
    if let Some(data_dir) = paths.data() {
        ...
        if zip_path.exists() {
            ...
            println!("Index time: {}ms", stats.get("last_zip_index_time_ms").unwrap_or(&0));
            println!("Files found: {}", stats.get("last_zip_2da_files_found").unwrap_or(&0));
        } else {
...
fn test_parallel_zip_indexing() {
    ...
    if let Some(data_dir) = paths.data() {
        ...
        if zip_paths.len() > 1 {
            ...
            let stats = indexer.get_stats();
            println!("Parallel index time: {}ms", 
                stats.get("last_parallel_zip_time_ms").unwrap_or(&0));
            println!("Zips processed: {}", 
