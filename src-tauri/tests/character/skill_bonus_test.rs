...
mod tests {
    ...
    async fn setup_test_environment() -> (Character, Arc<RwLock<ResourceManager>>, ItemPropertyDecoder) {
    ...
    async fn test_equipment_skill_bonus_application() {
        ...
        item.insert("Tag".to_string(), GffValue::String(Cow::Owned("TestHelmet".to_string())));
        ...
        assert_eq!(total, 10, "Skill bonus from equipment was not applied! Expected 10, got {}", total);
    }
}
