use crate::character::{
    AbilityIndex, AbilityScores, AbilityPointsSummary, Alignment, HitPoints, RaceId,
};
...
pub async fn set_character_age(state: State<'_, AppState>, age: i32) -> CommandResult<i32> {
    let mut session = state.session.write();
    let character = session.character.as_mut().ok_or(CommandError::NoCharacterLoaded)?;
    character.set_age(age).map_err(|e| CommandError::ValidationError {
        field: "age".to_string(),
        reason: e.to_string(),
    })?;
    Ok(character.age())
...
#[tauri::command]
pub async fn set_experience_points(
    state: State<'_, AppState>,
    xp: i32,
) -> CommandResult<i32> {
    let mut session = state.session.write();
...
pub async fn set_alignment(
    ...
    let mut session = state.session.write();
    let character = session.character.as_mut().ok_or(CommandError::NoCharacterLoaded)?;
    character.set_alignment(law_chaos, good_evil).map_err(|e| CommandError::ValidationError {
...
#[tauri::command]
pub async fn set_biography(state: State<'_, AppState>, description: String) -> CommandResult<String> {
    let mut session = state.session.write();
    let character = session.character.as_mut().ok_or(CommandError::NoCharacterLoaded)?;
    ...
    if let Some(domains_table) = game_data.get_table("domains") {
         for domain_id in domain_ids {
             if let Some(row) = domains_table.get_by_id(domain_id.0) {
                 let name_strref = row.get("Name").and_then(|v| v.as_ref()).and_then(|s| s.parse::<i32>().ok()).unwrap_or(-1);
                 let name = game_data.get_string(name_strref).unwrap_or_default();

                 let description_strref = row.get("Description").and_then(|v| v.as_ref()).and_then(|s| s.parse::<i32>().ok()).unwrap_or(-1);
...
pub async fn set_attribute(
    ...
    let mut session = state.session.write();
    let character = session.character.as_mut().ok_or(CommandError::NoCharacterLoaded)?;
    character.set_ability_with_cascades(ability, value, &game_data).map_err(|e| CommandError::ValidationError {
...
pub async fn set_all_ability_scores(
    ...
    let mut session = state.session.write();
    let character = session.character.as_mut().ok_or(CommandError::NoCharacterLoaded)?;
    character.set_ability_with_cascades(AbilityIndex::STR, scores.str_, &game_data).map_err(|e| CommandError::ValidationError {
        field: "STR".to_string(),
        reason: e.to_string(),
    })?;
    character.set_ability_with_cascades(AbilityIndex::DEX, scores.dex, &game_data).map_err(|e| CommandError::ValidationError {
        field: "DEX".to_string(),
        reason: e.to_string(),
    })?;
    character.set_ability_with_cascades(AbilityIndex::CON, scores.con, &game_data).map_err(|e| CommandError::ValidationError {
        field: "CON".to_string(),
        reason: e.to_string(),
    })?;
    character.set_ability_with_cascades(AbilityIndex::INT, scores.int, &game_data).map_err(|e| CommandError::ValidationError {
        field: "INT".to_string(),
        reason: e.to_string(),
    })?;
    character.set_ability_with_cascades(AbilityIndex::WIS, scores.wis, &game_data).map_err(|e| CommandError::ValidationError {
        field: "WIS".to_string(),
        reason: e.to_string(),
    })?;
    character.set_ability_with_cascades(AbilityIndex::CHA, scores.cha, &game_data).map_err(|e| CommandError::ValidationError {
        field: "CHA".to_string(),
        reason: e.to_string(),
    })?;
    Ok(())
...
#[tauri::command]
pub async fn get_encumbrance_limits(state: State<'_, AppState>) -> CommandResult<EncumbranceLimits> {
    let session = state.session.read();
    let game_data = state.game_data.read();
    let character = session.character.as_ref().ok_or(CommandError::NoCharacterLoaded)?;
...
#[tauri::command]
pub async fn get_ability_points_summary(state: State<'_, AppState>) -> CommandResult<AbilityPointsSummary> {
    let session = state.session.read();
    let character = session.character.as_ref().ok_or(CommandError::NoCharacterLoaded)?;
...
#[tauri::command]
pub async fn get_ability_modifiers(state: State<'_, AppState>) -> CommandResult<crate::character::AbilityModifiers> {
    let session = state.session.read();
    let character = session.character.as_ref().ok_or(CommandError::NoCharacterLoaded)?;
...
#[tauri::command]
pub async fn get_racial_modifiers(state: State<'_, AppState>) -> CommandResult<crate::character::AbilityModifiers> {
    let session = state.session.read();
    let game_data = state.game_data.read();
    let character = session.character.as_ref().ok_or(CommandError::NoCharacterLoaded)?;
