...
static RE_ALIGNMENT: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)<b>\s*Alignment:?\s*</b>").expect("Invalid regex"));
static RE_PORTFOLIO: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)<b>\s*Portfolio:?\s*</b>").expect("Invalid regex"));
static RE_ALIASES: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)<b>\s*Aliases:?\s*</b>").expect("Invalid regex"));
static RE_WEAPON: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)<b>\s*Favored Weapon:?\s*</b>").expect("Invalid regex"));
static RE_STRIP_HTML: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"<[^>]+>").expect("Invalid regex"));
static RE_NEWLINES: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\n\s*\n").expect("Invalid regex"));
static RE_TITLE_STRIP: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)^\s*(?:<color=[^>]+>\s*)?<b>\s*[^<]+?\s*</b>(?:</color>)?\s*").expect("Invalid regex"));

#[tauri::command]
pub async fn get_tlk_string(
    state: State<'_, AppState>,
    str_ref: u32,
) -> CommandResult<String> {
    let game_data = state.game_data.read();
    game_data.get_string(str_ref as i32)
        .ok_or_else(|| CommandError::NotFound { item: format!("String ref {str_ref}") })
}
...
pub async fn get_2da_row(
    ...
    let game_data = state.game_data.read();
    let table = game_data.get_table(&table_name)
        .ok_or_else(|| CommandError::NotFound { item: format!("Table {table_name}") })?;

    let row = table.get_row(row_index)
        .map_err(|_| CommandError::NotFound { item: format!("Row {row_index} in table {table_name}") })?;
...
pub async fn list_2da_tables(state: State<'_, AppState>) -> CommandResult<Vec<String>> {
    let game_data = state.game_data.read();
    Ok(game_data.table_names().map(std::string::ToString::to_string).collect())
}
...
pub async fn get_2da_table(
    ...
    for i in 0..table.row_count() {
        if let Some(row) = table.get_by_id(i as i32) {
            let standard_map: HashMap<String, Option<String>> =
...
#[tauri::command]
pub async fn get_available_classes(state: State<'_, AppState>) -> CommandResult<Vec<AvailableClass>> {
    let game_data = state.game_data.read();
    let table = game_data
        .get_table("classes")
        .ok_or_else(|| CommandError::NotFound { item: "Classes table".to_string() })?;
...
pub async fn get_available_feats(state: State<'_, AppState>) -> CommandResult<Vec<AvailableFeat>> {
    ...
    let table = game_data
        .get_table("feat")
        .ok_or_else(|| CommandError::NotFound { item: "Feat table".to_string() })?;
...
pub async fn initialize_game_data(state: State<'_, AppState>) -> CommandResult<bool> {
...

    {
        ...
        if table_count > 0 {
            info!("Game data already initialized ({} tables loaded)", table_count);
            return Ok(true);
        }
    }
    ...
    info!("Game data initialization completed: {} tables loaded", table_count);
...
#[tauri::command]
pub async fn get_available_skills(state: State<'_, AppState>) -> CommandResult<Vec<AvailableSkill>> {
    let game_data = state.game_data.read();
    let table = game_data
        .get_table("skills")
        .ok_or_else(|| CommandError::NotFound { item: "Skills table".to_string() })?;
...
#[tauri::command]
pub async fn get_available_spells(state: State<'_, AppState>) -> CommandResult<Vec<AvailableSpell>> {
    let game_data = state.game_data.read();
    let table = game_data
        .get_table("spells")
...
pub async fn get_available_races(state: State<'_, AppState>) -> CommandResult<Vec<AvailableRace>> {
    ...
    let table = game_data
        ...
        .ok_or_else(|| CommandError::NotFound { item: "Racialtypes table".to_string() })?;
...
#[tauri::command]
pub async fn get_available_genders(state: State<'_, AppState>) -> CommandResult<Vec<AvailableGender>> {
    let game_data = state.game_data.read();
    let table = game_data
        .get_table("gender")
        .ok_or_else(|| CommandError::NotFound { item: "Gender table".to_string() })?;
...
#[tauri::command]
pub async fn get_available_alignments(_state: State<'_, AppState>) -> CommandResult<Vec<AvailableAlignment>> {
    Ok(vec![
        AvailableAlignment { id: 0, name: "Lawful Good".to_string(), law_chaos: 0, good_evil: 0 },
        AvailableAlignment { id: 1, name: "Neutral Good".to_string(), law_chaos: 50, good_evil: 0 },
        AvailableAlignment { id: 2, name: "Chaotic Good".to_string(), law_chaos: 100, good_evil: 0 },
        AvailableAlignment { id: 3, name: "Lawful Neutral".to_string(), law_chaos: 0, good_evil: 50 },
        AvailableAlignment { id: 4, name: "True Neutral".to_string(), law_chaos: 50, good_evil: 50 },
        AvailableAlignment { id: 5, name: "Chaotic Neutral".to_string(), law_chaos: 100, good_evil: 50 },
        AvailableAlignment { id: 6, name: "Lawful Evil".to_string(), law_chaos: 0, good_evil: 100 },
        AvailableAlignment { id: 7, name: "Neutral Evil".to_string(), law_chaos: 50, good_evil: 100 },
        AvailableAlignment { id: 8, name: "Chaotic Evil".to_string(), law_chaos: 100, good_evil: 100 },
    ])
}
...
#[tauri::command]
pub async fn get_available_deities(state: State<'_, AppState>) -> CommandResult<Vec<AvailableDeity>> {
    let game_data = state.game_data.read();
    ...
    for i in 0..table.row_count() {
        ...
        deities.push(AvailableDeity {
            ...
            name,
            alignment: if parsed.alignment.is_empty() { None } else { Some(parsed.alignment) },
            portfolio: if parsed.portfolio.is_empty() { None } else { Some(parsed.portfolio) },
            description: if parsed.description.is_empty() { None } else { Some(parsed.description) },
            icon: if icon.is_empty() { None } else { Some(icon) },
            aliases: if parsed.aliases.is_empty() { None } else { Some(parsed.aliases) },
            favored_weapon: if parsed.favored_weapon.is_empty() { None } else { Some(parsed.favored_weapon) },
        });
    }
    ...
    if let Some(first) = deities.first() {
        debug!("First deity: {} (Desc len: {}, Align: {:?})", 
            first.name, 
...
pub async fn get_all_domains(state: State<'_, AppState>) -> CommandResult<Vec<AvailableDomain>> {
    ...
    let table = game_data
        .get_table("domains")
        .ok_or_else(|| CommandError::NotFound { item: "Domains table".to_string() })?;
...
#[tauri::command]
pub async fn get_available_backgrounds(state: State<'_, AppState>) -> CommandResult<Vec<AvailableBackground>> {
    let game_data = state.game_data.read();
...
#[tauri::command]
pub async fn get_available_abilities(state: State<'_, AppState>) -> CommandResult<Vec<AvailableAbility>> {
    let game_data = state.game_data.read();
    let abilities = vec![
        (0, 135, "Strength", "STR"),
        (1, 133, "Dexterity", "DEX"),
    ...
    let result = abilities.into_iter().map(|(id, strref, default_name, short_name)| {
        let name = game_data.get_string(strref).unwrap_or_else(|| default_name.to_string());
        AvailableAbility {
            id,
            name,
...
#[tauri::command]
pub async fn get_available_base_items(state: State<'_, AppState>) -> CommandResult<Vec<AvailableBaseItem>> {
    let game_data = state.game_data.read();
    let table = game_data
        .get_table("baseitems")
        .ok_or_else(|| CommandError::NotFound { item: "Baseitems table".to_string() })?;
...
#[tauri::command]
pub async fn get_available_spell_schools(state: State<'_, AppState>) -> CommandResult<Vec<AvailableSpellSchool>> {
    let game_data = state.game_data.read();
    let table = game_data
        .get_table("spellschools")
        .ok_or_else(|| CommandError::NotFound { item: "Spellschools table".to_string() })?;
...
#[tauri::command]
pub async fn get_available_item_properties(state: State<'_, AppState>) -> CommandResult<Vec<AvailableItemProperty>> {
    let game_data = state.game_data.read();
    let table = game_data
        .get_table("itempropdef")
        .ok_or_else(|| CommandError::NotFound { item: "Itempropdef table".to_string() })?;
...
";
        ...
        assert_eq!(parsed.aliases, "The Lady of Mysteries, The Mother of All Magic");
        assert_eq!(parsed.favored_weapon, "Shuriken");
        assert_eq!(parsed.description, "Mystra (MISS-trah) is the goddess of magic.");
    }
