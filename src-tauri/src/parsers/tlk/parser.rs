            ...
            if let Ok(cached_data) = self.load_from_cache(cache_path) {
                ...
                if let Ok(source_metadata) = std::fs::metadata(source_path)
                    && let Ok(cache_metadata) = std::fs::metadata(cache_path)
                        && cache_metadata.modified().unwrap_or(std::time::SystemTime::UNIX_EPOCH) 
                           >= source_metadata.modified().unwrap_or(std::time::SystemTime::UNIX_EPOCH) {
                            *self = cached_data;
...
pub fn load_multiple_files(paths: &[&str], limits: Option<SecurityLimits>) -> TLKResult<HashMap<String, TLKParser>> {
    let results: Result<Vec<_>, _> = paths.par_iter()
        .map(|&path| {
            ...
            parser.parse_from_file(path).map(|()| (path.to_string(), parser))
        })
        .collect();
