...
pub fn discover_nwn2_paths_rust(search_paths: Option<Vec<String>>) -> Result<DiscoveryResult, String> {
            ...
            && let Ok(entries) = std::fs::read_dir(&search_path) {
                for entry in entries.flatten() {
                    if let Ok(file_type) = entry.file_type()
                        && file_type.is_dir()
                            && let Some(name) = entry.file_name().to_str()
                                && name.to_uppercase().contains("NWN2") && is_nwn2_installation(&entry.path()) {
                                    let path_str = entry.path().to_string_lossy().to_string();
                                    nwn2_paths.push(path_str.clone());
                                    paths_found += 1;
