...
pub fn parse_journal_gff(data: &[u8], source: &str) -> Result<HashMap<String, QuestDefinition>, String> {
    let gff = GffParser::from_bytes(data.to_vec()).map_err(|e| format!("Failed to parse journal GFF: {e}"))?;
        ...
        for category in categories {
            let cat_fields = category.force_load();
                ...
                let name = match cat_fields.get("Name") {
                    Some(GffValue::LocString(ls)) => ls.substrings.first().map_or_else(|| tag.clone(), |sub| sub.string.to_string()),
                    Some(GffValue::String(s)) => s.to_string(),
                    _ => tag.clone(),
                };
                ...
                if let Some(GffValue::List(entries)) = cat_fields.get("EntryList") {
                    for entry in entries {
                        ...
                        definitions.insert(key, QuestDefinition {
