...
async fn test_races_table_loaded() {
    ...
    let table = ctx.loader.get_table("racialtypes")
...
async fn test_races_columns() {
    ...
    let table = ctx.loader.get_table("racialtypes").expect("racialtypes.2da missing");
    ...
    let required_cols = ["Label", "StrAdjust", "DexAdjust", "ConAdjust", 
                         "IntAdjust", "WisAdjust", "ChaAdjust", "Favored"];
...
async fn test_human_no_adjustments() {
    ...
    let human_row = (0..table.row_count()).find(|&row| {
...
async fn test_dwarf_ability_adjustments() {
    ...
    let dwarf_row = (0..table.row_count()).find(|&row| {
        cell_value(table, row, "Label")
...
async fn test_playable_races_count() {
    ...
    if has_player_race {
        let playable_count = (0..table.row_count())
            .filter(|&row| {
                cell_int(table, row, "PlayerRace").unwrap_or(0) == 1
            })
            .count();
...
async fn test_subrace_columns() {
    ...
    if let Some(table) = ctx.loader.get_table("racialsubtypes") {
        ...
        assert!(cols.iter().any(|c| c.eq_ignore_ascii_case("Label")), "Should have Label column");
    }
}
...
async fn test_playable_subraces() {
    ...
    if let Some(table) = ctx.loader.get_table("racialsubtypes") {
        ...
        let player_col = cols.iter().find(|c| 
            c.eq_ignore_ascii_case("PlayerRace") || 
            c.eq_ignore_ascii_case("Playable")
        );
        
        if let Some(col) = player_col {
            let playable_count = (0..table.row_count())
                .filter(|&row| {
                    cell_int(table, row, col).unwrap_or(0) == 1
                })
                .count();
