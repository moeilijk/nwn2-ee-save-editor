...
async fn test_build_prerequisite_graph_from_real_feats() {
    ...
    if let Some(table) = ctx.loader.get_table("feat") {
        ...
        let total_feats = stats.get("total_feats").and_then(|v| v.as_u64()).unwrap_or(0);
        assert!(total_feats > 100, "Should have many feats (got {})", total_feats);
    }
}
    ...
    if let Some(table) = ctx.loader.get_table("feat") {
        ...
        if let (Some(pa_idx), Some(cleave_idx)) = (power_attack_row, cleave_row) {
            ...
            if let Some(prereq) = cleave_prereq {
                assert_eq!(prereq as usize, pa_idx, "Cleave should require Power Attack");
...
async fn test_validate_feat_with_real_graph() {
    ...
    if let Some(table) = ctx.loader.get_table("feat") {
        ...
        if let (Some(cleave_idx), Some(pa_idx)) = (cleave_row, power_attack_row) {
            let (valid_without, reasons) = graph.validate_feat_prerequisites_fast(
                cleave_idx as u32, 
                &[], 
                None
            );
            ...
            let (valid_with, _) = graph.validate_feat_prerequisites_fast(
                cleave_idx as u32, 
                &[pa_idx as u32], 
                None
            );
...
async fn test_get_feat_requirements_chain() {
    ...
    if let Some(table) = ctx.loader.get_table("feat") {
        ...
        if let Some(gc_idx) = great_cleave_row {
            ...
            println!("Great Cleave (feat {}) requires feats: {:?}", gc_idx, requirements);
...
async fn test_batch_validation_performance() {
    ...
    if let Some(table) = ctx.loader.get_table("feat") {
        ...
        println!("Batch validated {} feats in {:?}", test_feats.len(), duration);
        println!("Results: {} valid, {} invalid",
...
async fn test_graph_statistics() {
    ...
    if let Some(table) = ctx.loader.get_table("feat") {
        ...
        assert!(stats.get("total_feats").and_then(|v| v.as_u64()).unwrap_or(0) > 0);
        assert!(stats.get("feats_with_prerequisites").and_then(|v| v.as_u64()).unwrap_or(0) > 0);
        assert!(stats.get("max_chain_depth").and_then(|v| v.as_u64()).unwrap_or(0) > 0);
        
        println!("Graph built with:");
        println!("  Total feats: {}", stats.get("total_feats").unwrap());
        println!("  With prereqs: {}", stats.get("feats_with_prerequisites").unwrap());
