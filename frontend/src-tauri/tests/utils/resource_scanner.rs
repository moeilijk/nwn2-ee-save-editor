...
fn test_scan_nwn2_data_zips() {
    ...
    if let Some(data_dir) = paths.data() {
        ...
        assert!(resources.contains_key("classes.2da"), "Should find classes.2da");
        assert!(resources.contains_key("feat.2da"), "Should find feat.2da");
        assert!(resources.contains_key("skills.2da"), "Should find skills.2da");
    }
}
...
fn test_scan_workshop_directories() {
    ...
    if let Some(workshop_dir) = paths.steam_workshop_folder() {
        if workshop_dir.exists() {
            let mut scanner = ResourceScanner::new();
            let result = scanner.scan_workshop_directories(vec![
                workshop_dir.to_string_lossy().to_string()
            ]);
...
fn test_index_override_directory() {
    ...
    if let Some(override_dir) = paths.override_dir() {
        let mut scanner = ResourceScanner::new();
        let result = scanner.index_directory(
...
fn test_comprehensive_scan() {
    ...
    let workshop_dirs: Vec<String> = paths.steam_workshop_folder()
        .filter(|p| p.exists())
        .map(|p| vec![p.to_string_lossy().to_string()])
        .unwrap_or_default();
    
    let override_dirs: Vec<String> = paths.override_dir()
        .filter(|p| p.exists())
        .map(|p| vec![p.to_string_lossy().to_string()])
        .unwrap_or_default();
    
    let enhanced_dir = paths.enhanced_data()
        .map(|p| p.to_string_lossy().to_string());
...
fn test_resource_location_types() {
    ...
    if let Ok(scan_results) = result {
        let zip_resources = scan_results.resource_locations.values()
            .filter(|loc| loc.source_type == "zip")
            .count();
        
        let file_resources = scan_results.resource_locations.values()
...
fn test_scan_finds_core_2da_tables() {
    ...
    if let Ok(scan_results) = result {
        let core_tables = [
            "classes.2da", "feat.2da", "skills.2da", "spells.2da",
            "racialtypes.2da", "baseitems.2da", "appearance.2da"
        ];
...
fn test_enhanced_edition_scanning() {
    ...
    if let Some(enhanced_data) = paths.enhanced_data() {
        if enhanced_data.exists() {
            ...
            println!("Enhanced Edition scan found {} resources", scan_results.resources_found);
        } else {
